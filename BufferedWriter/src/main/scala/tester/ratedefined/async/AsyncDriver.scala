...
import lib.{AsyncBufferedWriter, SyncBufferedWriter}
 ...
 */
class AsyncDriver(config: TestConfig) {
  ...
  def run(): Unit = {
    ...
    for (i <- 0 until config.consumerCount) {
      ...
      val consumer = new AsyncConsumer(i, queue, bufferedWriter, stats, producerDone, consumersDone)
    ...
    // Wait for a moment for all futures to complete. I am not sure why this is needed. bufferedWriter.shutdown() should
    // be enough
