...
import com.vmware.kvstore.parser.Command;
import com.vmware.kvstore.store.RootStore;
import com.vmware.kvstore.store.Transaction;
...
public class Engine {
    ...
    private void executeStart() {
        activeTransactions.offer(new Transaction<>());
    }
    ...
    public void execute(final Command command, final String key, final String value) throws IOException {
        if (command == Command.WRITE) {
            ...
            try {
                ...
                os.write(String.format("%s", v).getBytes(StandardCharsets.UTF_8));
            } catch (KeyDeletedException | KeyNotFoundException e) {
                os.write(String.format("Key not found: %s", key).getBytes(StandardCharsets.UTF_8));
            }
        } else if (command == Command.DELETE) {
            try {
                executeDelete(key);
            } catch (KeyNotFoundException e) {
                os.write(String.format("Key not found: %s", key).getBytes());
            }
